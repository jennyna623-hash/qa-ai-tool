...
- BUG 標題與 Jira 描述產生
- BUG 版型新增「新架構」並設為 STG 預設選項；`https://gsi2.gsiwl.com/` 歸屬「新架構」，不再歸屬 R017
- 固定受託人清單，預設 Edward
- 受託人與主單不寫入描述
...
- 周進度輸入 Jira 單號後，自動讀取標題、狀態與分類，也可手動新增「標題＋連結」；彙整區只保留分類、數量與工作連結，不輸出日期、人員或細項進度；複製到 Notion 時，分類標題會加粗，項目以縮排編號緊湊排列；填寫人可選 Jenny、Ben 或 Guan
- 每位同事的瀏覽器個人設定
...
- 需求單回報可一次輸入最多 20 筆 Jira 單號；多筆結果會彙整為編號需求清單，共用一組狀態與 CC，並支援 DEV／STG 完成與驗退文案、Telegram 帳號對照、編輯及富文字複製
...
- 進度追蹤可依目前篩選結果產生每週測試報告，預覽完成率、環境與進度分布、風險、結論及明細，並下載雙工作表 Excel 或列印儲存為 PDF
...
多人共用進度另需在 Pages 專案的 Bindings 新增 D1 database：

- Variable name：`PROGRESS_DB`
- D1 database：`qa-ai-tool-progress`
